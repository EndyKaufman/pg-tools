...
export class CreateDatabaseService {
  ...
  replaceEnv(command: string | undefined, envReplacerKeyPattern = '$key', depth = 10): string {
    if (!command) {
      return command || '';
    }
  ...
  async createDatabase({
    ...
    rootDatabaseUrl = this.replaceEnv(rootDatabaseUrl);
    appDatabaseUrl = this.replaceEnv(appDatabaseUrl);
