...
import { DEFAULT_MIGRATE_CONFIG, HistoryTableService, MigrateService, Migration } from 'pg-flyway';
...
describe('Basic migrate with pglite and migration files in different locations', () => {
  ...
  beforeAll(async () => {
    ...
    await saveBasicMigrationsToFileSystem(
      join(__dirname, 'basic-migrate-with-pglite-and-migration-files-in-different-locations')
    );
  ...
  it('Check migration history table', async () => {
    const migrations = (
      await migrateService.execSqlForStatments({
        databaseUrl: pg.connectionString,
        migration: Migration.fromStatements({
          statements: [historyTableService.getMigrationsHistorySql()],
        }),
        placeholders: {},
      })
    ).flat();
