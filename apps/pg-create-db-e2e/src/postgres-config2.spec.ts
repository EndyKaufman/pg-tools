...
describe('Postgres config 2 (e2e)', () => {
  ...
  it('create applications databases with config file', async () => {
    await execa('node', ['dist/libs/pg-create-db/index.js', `--config=./apps/pg-create-db-e2e/src/pg-create-db2`], {
      env: {
        HOST: container.getHost(),
        PORT: String(container.getMappedPort(5432)),
        PG_CREATE_DB_ROOT_DATABASE_URL: `postgres://${ROOT_POSTGRES_USER}:${ROOT_POSTGRES_PASSWORD}@${container.getHost()}:${container.getMappedPort(
          5432
